...
def page():
    intro()
    st.subheader("Escoge el tema a desplegar")
    opcion = st.selectbox(
        "",
        (
    ...
    if opcion == 'Definiciones importantes':
        importante_saber()
        tabsArbol()
    if opcion == 'Tipos de monticulos':
        monticulosMaxMin()
        tipos()
    if opcion =='Monticulos MaxMin':
        monticulosMaxMin()
    if opcion == 'Ejercicios básicos de comprensión':
        ejerciciospracticos()
    if opcion == 'Ejercicio de aplicación':
        st.write('EJERCICIO AQUÍ')


def intro():
    st.header('⛰️')
    st.write('<div style="text-align: center;"><h1>Monticulos</h1></div>', unsafe_allow_html=True)
    st.write(
        """
...
def importante_saber():
    ...
    st.write("<h2>Importante entender</h2>", unsafe_allow_html=True)
    st.write("Los monticulos se componen de los siguientes elementos:", unsafe_allow_html=True)

    col1, col2, col3 = st.columns(3)

    with col1:
        st.write("<b>Nodo</b>: cada elemento que contiene un árbol", unsafe_allow_html=True)
        st.write("<b>Nodo raíz</b>: primer nodo de un árbol", unsafe_allow_html=True)
        st.write("<b>Nodo padre</b>: aquellos nodos que tienen al menos un hijo y está directamente superior a el", unsafe_allow_html=True)
        st.write("<b>Nodo hijo</b>: todos quellos que tienen un padre", unsafe_allow_html=True)    
        st.write("<b>Nodo hermano</b>: los nodos que comparten a un mismo padre", unsafe_allow_html=True)
        st.write("<b>Nodo hoja</b>: nodos que no tienen hijos", unsafe_allow_html=True)
        st.write("<b>Nodo rama</b>: aquellos nodos que no son raíz y tienen al menos un hijo", unsafe_allow_html=True)
        st.write("<b>Decendiente</b>: nodo que está en un nivel inferior en relación con otro nodo específico", unsafe_allow_html=True)
    with col2:
        st.write("<b>Ancestro</b>: nodo que está en un nivel superior en relación con otro nodo específico", unsafe_allow_html=True)
        st.write("<b>Nodo interno</b>: nodo que tiene al menos un hijo", unsafe_allow_html=True)
        st.write("<b>Camino</b>: secuencia de nodos conectados entre sí en el árbol", unsafe_allow_html=True)
        st.write("<b>Nivel de un nodo</b>: posición relativa de un nodo en el árbol", unsafe_allow_html=True)
        st.write("<b>Nivel de un árbol</b>: es el nivel más alto entre todos los nodos del árbol", unsafe_allow_html=True)
        st.write("<b>Altura de un nodo</b>: es la longitud del camino más largo desde ese nodo hasta un nodo terminal", unsafe_allow_html=True)
        st.write("<b>Altura de un árbol</b>: es la longitud máxima del camino desde la raíz hasta el nodo más alejado", unsafe_allow_html=True)
    with col3:
        st.write("<b>Peso de un árbol</b>: suma de los persos de todos los nodos de ese árbol", unsafe_allow_html=True)
        st.write("<b>Orden</b>: número máximo de hijos que puede tener un nodo", unsafe_allow_html=True)
        st.write("<b>Grado</b>: numero mayor d ehijos que tiene alguno de los nodos del árbol", unsafe_allow_html=True)
        st.write("<b>Profundidad</b>: distancia entre un nodo dado y la raíz del árbol", unsafe_allow_html=True)
        st.write("<b>Bosque</b>: coleccion de árboles disjuntos, donde cada átbol puede tener su propia raíz y nodos", unsafe_allow_html=True)
        st.write("<b>Sub-árbol</b>: es todo árbol generado a partir de una sección determinada del árbol", unsafe_allow_html=True)
        st.write("<b>Peso de un nodo</b>: valor asociado o atributo numérico asociado al nodo", unsafe_allow_html=True)
